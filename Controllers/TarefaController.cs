...
namespace TrilhaApiDesafio.Controllers
{
    ...
    public class TarefaController : ControllerBase
    {
        ...
        public IActionResult ObterPorData(DateTime data)
        {
          ...
          //Não foi pedido, mas vale um capricho no retorno que eu colocaria em todos os endpoints,
          //Para o programa ficar mais amigável, mas não vou colocar para não ficar extenso. Apenas uma cereja no bolo.
            var tarefaQtd = _context.Tarefas.Count(x => x.Data.Date == data.Date);
            if (tarefaQtd == 0)
               return BadRequest(new { Erro = "Não existe Tarefa para a data "+data.Day+"/"+data.Month+"/"+data.Year });
